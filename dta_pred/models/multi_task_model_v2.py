...
class MultiTaskModelV2():
    ...
    def train(self, datasets, checkpoint_callbacks, num_epoch, batch_size, **kwargs):
        for i in range(int(num_epoch*datasets[self.tasks[0]][0].shape[0]/batch_size)):
            task = self.tasks[math.ceil(np.random.uniform(0,3))]
            XD_train, XD_val, XD_test, XT_train, XT_val, XT_test, Y_train, Y_val, Y_test = datasets[task]

            gridres = self.compiled_models[task].fit(([XD_train, XT_train]), Y_train, batch_size=batch_size, epochs=1, steps_per_epoch=1,                  validation_data=(([np.array(XD_val), np.array(XT_val)]), np.array(Y_val)),
